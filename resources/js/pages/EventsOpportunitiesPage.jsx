...
const EventsOpportunitiesPage = () => {
  ...
  const filteredEvents = upcomingEvents.filter(e => {
    const matchType = selectedFilter === 'all' || e.type.toLowerCase() === selectedFilter;
  ...
  const filteredOpportunities = opportunities.filter(o =>
    !searchQuery || o.title.toLowerCase().includes(searchQuery.toLowerCase()) || o.company.toLowerCase().includes(searchQuery.toLowerCase())
  );
                  ...
                  <div className="event-footer">
                    ...
                    <div className="event-actions">
                      <button
                        ...
                        onClick={() => setSavedEvents(prev => ({ ...prev, [event.id]: !prev[event.id] }))}
                        style={savedEvents[event.id] ? { borderColor: '#EF4444', color: '#EF4444' } : {}}
                      >
                        <Heart size={16} />
                      </button>
                      <button className="action-icon" onClick={() => navigator.clipboard?.writeText(window.location.href).then(() => alert('Lien copié !'))}>
                        <Share2 size={16} />
                      </button>
                    </div>
                  </div>
          ...
          <div className="opportunities-list">
            {filteredOpportunities.map(opp => (
              <div key={opp.id} className={`opportunity-card ${opp.featured ? 'featured' : ''}`}>
                ...
                <button className="apply-btn" onClick={() => alert(`Candidature pour : ${opp.title} chez ${opp.company}\n\nFonctionnalité de candidature à connecter au backend.`)}>
                  Postuler
