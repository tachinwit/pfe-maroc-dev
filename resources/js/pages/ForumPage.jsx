...
const ForumPage = () => {
  ...
  const handleVote = (questionId, direction) => {
    setVotes(prev => {
      const current = prev[questionId] || { up: false, down: false };
      if (direction === 'up') return { ...prev, [questionId]: { up: !current.up, down: false } };
      return { ...prev, [questionId]: { up: false, down: !current.down } };
    });
  };

  const getVoteCount = (question) => {
    const v = votes[question.id];
    if (!v) return question.votes;
    if (v.up) return question.votes + 1;
    if (v.down) return question.votes - 1;
    return question.votes;
  };

  const filteredQuestions = questions
    .filter(q => {
      const matchCat = selectedCategory === 'all' || q.tags.some(t => t.toLowerCase().includes(selectedCategory));
      const matchSearch = !searchQuery || q.title.toLowerCase().includes(searchQuery.toLowerCase()) || q.content.toLowerCase().includes(searchQuery.toLowerCase());
      return matchCat && matchSearch;
    })
    .sort((a, b) => {
      if (sortBy === 'trending') return (b.trending ? 1 : 0) - (a.trending ? 1 : 0);
      if (sortBy === 'unanswered') return (a.solved ? 1 : 0) - (b.solved ? 1 : 0);
      return b.id - a.id;
    });
      ...
      <div className="forum-header">
        <div className="header-content">
          <div className="header-top">
            ...
            <button className="ask-btn" onClick={() => alert('Fonctionnalité de création de question — connectez-vous pour poster !')}>
              <Plus size={20} />
              Poser une Question
            </button>
          </div>
                ...
                <div className="question-header">
                  <div className="vote-section">
                    <button
                      ...
                      style={votes[question.id]?.up ? { borderColor: 'var(--cyan)', color: 'var(--cyan)' } : {}}
                    >
                      <ThumbsUp size={18} />
                    </button>
                    <div className="vote-count">{getVoteCount(question)}</div>
                    <button
                      className="vote-btn"
                      onClick={() => handleVote(question.id, 'down')}
                      style={votes[question.id]?.down ? { borderColor: '#EF4444', color: '#EF4444' } : {}}
                    >
                      <ThumbsDown size={18} />
                    </button>
                  </div>
