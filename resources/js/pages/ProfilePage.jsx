        ...
        <div className="header-content">
          ...
          <div className="profile-info">
            ...
            <div className="profile-actions">
              <button className="btn btn-primary" onClick={() => alert('Messagerie — fonctionnalité à venir')}>
                <MessageCircle size={18} />
                Envoyer un Message
              </button>
              <button className="btn btn-outline" onClick={(e) => { e.currentTarget.textContent = '✓ Suivi'; e.currentTarget.disabled = true; }}>
                <Users size={18} />
                Suivre
              </button>
              <div className="social-links">
                ...
                <a href="mailto:contact@devmaroc.ma" className="social-link">
                  <Mail size={20} />
                </a>
              </div>
            </div>
        ...
        <div>
          ...
          {activeTab === 'contributions' && (
            <div style={{ color: 'rgba(255,255,255,0.7)', lineHeight: '1.8' }}>
              {[
                { repo: 'laravel/framework', pr: 'Fix Eloquent eager loading N+1', status: 'Merged', date: '12 Jan 2025' },
                { repo: 'facebook/react', pr: 'Improve useCallback documentation', status: 'Merged', date: '5 Jan 2025' },
                { repo: 'tailwindlabs/tailwindcss', pr: 'Add RTL support for Arabic/Hebrew', status: 'Open', date: '28 Déc 2024' },
                { repo: 'inertiajs/inertia', pr: 'Fix memory leak in SPA navigation', status: 'Merged', date: '20 Déc 2024' },
              ].map((c, idx) => (
                <div key={idx} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', padding: '1.25rem 1.5rem', marginBottom: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontWeight: 700, color: '#00D9FF', fontSize: '0.85rem', marginBottom: '0.25rem' }}>{c.repo}</div>
                    <div style={{ color: '#fff', fontWeight: 600 }}>{c.pr}</div>
                    <div style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.5)', marginTop: '0.25rem' }}>{c.date}</div>
                  </div>
                  <span style={{ padding: '0.35rem 1rem', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 700, background: c.status === 'Merged' ? 'rgba(16,185,129,0.15)' : 'rgba(245,158,11,0.15)', color: c.status === 'Merged' ? '#10B981' : '#F59E0B', border: `1px solid ${c.status === 'Merged' ? 'rgba(16,185,129,0.3)' : 'rgba(245,158,11,0.3)'}` }}>{c.status}</span>
                </div>
              ))}
            </div>
          )}
